...
ADD headers-more-nginx-module-0.34.tar.gz /usr/local/
...
RUN set -e \
	...
	&& ./configure --add-module=/usr/local/ngx_http_proxy_connect_module-0.0.3 --add-module=/usr/local/headers-more-nginx-module-0.34 --prefix=/etc/nginx \
	&& make && make install \
